...
function Wishlist() {
  ...
  useEffect(() => {
    const apiUrl = `http://localhost:8080/wishlist/getWishlist/${userid}`;
    axios
      .get(apiUrl)
      .then((response) => {
  ...
  const removeWishlist = (placeName) => {
    const confirmed = window.confirm(
      `Are you sure you want to remove ${placeName} from your wishlist?`
    );
    if (confirmed) {
  ...
  const getNote = async (placeName) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/wishlist/getNote/${placeName}`
      );
  ...
  const handleNoteUpdate = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:8080/wishlist/updateWishList/${target}`,
        { note: editingNote }
      );
      setIsEditing(false);
  ...
  const generatePDFReport = () => {
    ...
    filteredPlaces.forEach((place) => {
      data.push([place.placeName, place.placeName2, place.description]);
    });
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <button
        type="button"
        className="text-white bg-gradient-to-r bg-[#30475c]  hover:bg-[#084469] text font-medium rounded-lg
      ...
      <div className="flex flex-col items-center">
        <h1 className="text-6xl font-bold mb-4 mt-4">My Trips</h1>
        ...
        <div>
          ...
          {Array.from(
            new Set(filteredPlaces.map((place) => place.placeName))
          ).map((placeName) => (
            <div key={placeName}>
              <div>
